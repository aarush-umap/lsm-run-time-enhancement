...
class Denoiser(nn.Module):
    ...
    def configure_dataset(self):
        config = self.config
        train_csv_path, valid_csv_path = generate_compress_csv(dataset=config['dataset'], ext=str(config['image-extension']))
    ...
    def train_epoch(self, epoch=1, total_epoch=1):
        ...
        for iteration, batch in enumerate(dataloader):
            ...
            print(f'[{epoch}/{total_epoch}] [{iteration}/{len(dataloader)}] Loss: {loss_pixel.item()}', end='\r')
        print(f"\n ===> Epoch {epoch} Complete: Avg. Loss: {epoch_loss / len(dataloader):.6f}")
        self.log_train.append((epoch_loss/len(dataloader), epoch))
    ...
    def denoise(self, sampling=False, sample_rate=1, batch_size=50):
        with torch.no_grad():
            ...
            noisy_path = os.path.join('output', config['dataset'], 'noisy')
            clean_path = os.path.join('output', config['dataset'], 'clean')
